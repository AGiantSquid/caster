...
DIRECTION_STANDARD = {"Jeep [E]": "up", "doom [E]": "down",
                      "crimp [E]": "left", "Ross [E]": "right", "back": "left"}
...
def clipboard_to_file(nnavi500, nexus, do_copy=False):
    ...
    for i in range(0, max_tries):
        ...
        try:
            ...
            time.sleep(settings.SETTINGS["miscellaneous"][
                       "keypress_wait"] / 1000.)
            win32clipboard.OpenClipboard()
            nexus.clip[key] = win32clipboard.GetClipboardData()
            win32clipboard.CloseClipboard()
            utilities.save_json_file(nexus.clip, settings.SETTINGS[
                                     "paths"]["SAVED_CLIPBOARD_PATH"])
        except Exception:
            failure = True
...
def wheel_scroll_down(nnavi500):
    wheel_scroll('down', nnavi500)


def wheel_scroll_up(nnavi500):
    wheel_scroll('up', nnavi500)
