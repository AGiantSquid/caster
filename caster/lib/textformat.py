...
def set_text_format(capitalization, spacing):
    ...
    _SPACING = spacing
    print("Text formatting: %s" %
          get_text_format_description(_CAPITALIZATION, _SPACING))
...
def get_text_format_description(capitalization, spacing):
    ...
    if capitalization == 0 and spacing == 0:
        ...
        text = get_formatted_text(
            capitalization, spacing, str("this is a test"))
...
def format_text_wrapper(cap, space, textnv):
    ...
    in nav.py
    '''
    master_format_text(cap, space, textnv)
